...
void draw(stlxgfx_surface_t* back, const dm_window& w, bool focused,
    ...
    /* A view surface over the clip rect makes every primitive clip
     * for free, chrome coordinates shift into view space below */
    ...
    int32_t wx = w.x - clip.x;
    int32_t wy = w.y - clip.y;
    ...
    stlxgfx_fill_rect(view, wx - BORDER, wy - TITLE_H,
                      static_cast<uint32_t>(bw + 2 * BORDER),
                      static_cast<uint32_t>(BORDER), border);
    stlxgfx_fill_rect(view, wx - BORDER, wy + bh,
                      static_cast<uint32_t>(bw + 2 * BORDER),
                      static_cast<uint32_t>(BORDER), border);
    stlxgfx_fill_rect(view, wx - BORDER, wy - TITLE_H,
                      static_cast<uint32_t>(BORDER),
                      static_cast<uint32_t>(TITLE_H + bh + BORDER), border);
    stlxgfx_fill_rect(view, wx + bw, wy - TITLE_H,
                      static_cast<uint32_t>(BORDER),
                      static_cast<uint32_t>(TITLE_H + bh + BORDER), border);

    /* Title bar with the window name and the close control */
    stlxgfx_fill_rect(view, wx, wy - TITLE_H + BORDER,
                      static_cast<uint32_t>(bw),
                      static_cast<uint32_t>(TITLE_H - BORDER), title_bg);
    stlxgfx_draw_text(view, g_font, wx + 10,
                      wy - TITLE_H + 7 + g_fm.ascent,
                      w.title, strlen(w.title), title_fg);

    if (focused) {
        int32_t ccx = wx + bw - CLOSE_MARGIN;
        int32_t ccy = wy - TITLE_H / 2;
        stlxgfx_fill_circle(view, ccx, ccy, static_cast<uint32_t>(CLOSE_R),
