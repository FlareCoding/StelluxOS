...
public:
    ...
    int open(const char* iface, const uint8_t* mac, bool verbose);

    int fd() const { return m_fd; }
    bool bound() const { return m_state == dhcp_state::bound; }
...
private:
    ...
    void transmit(const dhcp_message& msg, const char* what);
    ...
    uint32_t   m_xid = 0;
    uint64_t   m_acquisition_start_ns = 0;
