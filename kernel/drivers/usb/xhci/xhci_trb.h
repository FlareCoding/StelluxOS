#ifndef STELLUX_DRIVERS_USB_XHCI_XHCI_TRB_H
#define STELLUX_DRIVERS_USB_XHCI_XHCI_TRB_H
...
typedef struct xhci_set_tr_dequeue_ptr_command_request_block {
    uint64_t new_dequeue_ptr;
    uint32_t rsvd0;
    struct {
        uint32_t cycle_bit    : 1;
        uint32_t rsvd1        : 9;
...
static_assert(sizeof(xhci_set_tr_dequeue_ptr_command_trb_t) == sizeof(uint32_t) * 4);
...
#endif // STELLUX_DRIVERS_USB_XHCI_XHCI_TRB_H
