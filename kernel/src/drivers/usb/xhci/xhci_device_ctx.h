...
struct XhciSlotContext {
    ...
    */
    ...
    uint32_t rsvdZ[Size == 64 ? 12 : 4]; // 12 reserved for 64-byte, 4 for 32-byte
} __attribute__((packed));

using XhciSlotContext32 = XhciSlotContext<32>;
static_assert(sizeof(XhciSlotContext32) == 32, "Size mismatch for 32-byte Slot Context.");

using XhciSlotContext64 = XhciSlotContext<64>;
static_assert(sizeof(XhciSlotContext64) == 64, "Size mismatch for 64-byte Slot Context.");
...
struct XhciEndpointContext {
    ...
    // Reserved fields depending on context size
    uint32_t rsvd[Size == 64 ? 8 : 0]; // 8 reserved for 64-byte context, none for 32-byte
};

using XhciEndpointContext32 = XhciEndpointContext<32>;
static_assert(sizeof(XhciEndpointContext32) == 32, "Size mismatch for 32-byte Endpoint Context.");

using XhciEndpointContext64 = XhciEndpointContext<64>;
static_assert(sizeof(XhciEndpointContext64) == 64, "Size mismatch for 64-byte Endpoint Context.");
...
static_assert(sizeof(XhciDeviceContext32) == 1024, "Size mismatch for 32-byte Device Context.");
