...
void VGATextDriver::init(uint32_t width, uint32_t height, uint32_t pixelsPerScanline, void* font) {
    ...
    s_font = reinterpret_cast<Psf1Font*>(font);
}
...
void VGATextDriver::_renderChar(char chr, uint32_t color) {
    ...
    if (s_cursorPosY + charPixelHeight > s_height) {
        ...
        for (unsigned long offy = charPixelHeight; offy < s_height; ++offy) {
        ...
        // Clear the last line
        for (unsigned long offy = s_height - charPixelHeight; offy < s_height; ++offy) {
    ...
    for (unsigned long yOffset = s_cursorPosY; yOffset < s_cursorPosY + charPixelHeight; yOffset++) {
