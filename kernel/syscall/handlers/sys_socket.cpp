...
// Translate resource layer error codes to syscall errno values.
int64_t translate_err(int32_t rc) {
    switch (rc) {
    case resource::OK:              return 0;
...
DEFINE_SYSCALL3(bind, fd, addr, addrlen) {
    ...
    if (obj->type != resource::resource_type::SOCKET || !obj->ops || !obj->ops->bind) {
        resource::resource_release(obj);
        return syscall::EOPNOTSUPP;
    }

    // Copy sockaddr from userspace
    size_t alen = static_cast<size_t>(addrlen);
    if (addr == 0 || alen < sizeof(uint16_t) || alen > SENDTO_MAX_ADDR) {
        resource::resource_release(obj);
        return syscall::EINVAL;
    }
    uint8_t kaddr[SENDTO_MAX_ADDR] = {};
    int32_t copy_rc = mm::uaccess::copy_from_user(
        kaddr, reinterpret_cast<const void*>(addr), alen);
    if (copy_rc != mm::uaccess::OK) {
        resource::resource_release(obj);
        return syscall::EFAULT;
    }

    rc = obj->ops->bind(obj, kaddr, alen);
    resource::resource_release(obj);
    return translate_err(rc);
}
...
DEFINE_SYSCALL2(listen, fd, backlog) {
    ...
    if (obj->type != resource::resource_type::SOCKET || !obj->ops || !obj->ops->listen) {
        resource::resource_release(obj);
        return syscall::EOPNOTSUPP;
    }

    rc = obj->ops->listen(obj, static_cast<int32_t>(backlog));
    resource::resource_release(obj);
    return translate_err(rc);
}
...
DEFINE_SYSCALL3(connect, fd, addr, addrlen) {
    ...
    if (obj->type != resource::resource_type::SOCKET || !obj->ops || !obj->ops->connect) {
        resource::resource_release(obj);
        return syscall::EOPNOTSUPP;
    }
    ...
    rc = obj->ops->connect(obj, kaddr, alen);
    resource::resource_release(obj);
    return translate_err(rc);
}
...
DEFINE_SYSCALL3(accept, fd, addr, addrlen) {
    ...
    if (obj->type != resource::resource_type::SOCKET || !obj->ops || !obj->ops->accept) {
        resource::resource_release(obj);
        return syscall::EOPNOTSUPP;
    }
    ...
    resource::resource_object* new_conn = nullptr;
    uint8_t kaddr[SENDTO_MAX_ADDR] = {};
    size_t kaddr_len = sizeof(kaddr);

    rc = obj->ops->accept(obj, &new_conn, kaddr, &kaddr_len, nonblock);
    if (rc != resource::OK) {
        resource::resource_release(obj);
        return translate_err(rc);
    }

    // Install a handle for the accepted connection
    resource::handle_t new_handle = -1;
    rc = resource::alloc_handle(
        &task->handles, new_conn, resource::resource_type::SOCKET,
