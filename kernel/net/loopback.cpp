...
static int32_t lo_transmit(netif* iface, const uint8_t* frame, size_t len) {
    ...
    //
    // Safety: drain_deferred_tx() calls ipv4_send() which may call
    // back into lo_transmit() → rx_frame(). This does NOT cause
    // infinite recursion because:
    //   - The echo REPLY generated by icmp_recv is type ECHO_REPLY
    //   - icmp_recv only queues deferred TX for ECHO_REQUEST type
    //   - So the reply just gets delivered to sockets, no new deferred TX
    drain_deferred_tx();
