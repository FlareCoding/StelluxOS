...
struct congestion_ops {
    const char* name;
    void        (*init)(tcp_conn* conn);
    void        (*release)(tcp_conn* conn);
    uint32_t    (*ssthresh)(tcp_conn* conn);
    void        (*grow)(tcp_conn* conn, uint32_t acked_bytes);
    void        (*set_state)(tcp_conn* conn, recovery_state state);
    void        (*event)(tcp_conn* conn, congestion_event which);
    void        (*acked)(tcp_conn* conn, uint32_t bytes, int64_t rtt_us);
